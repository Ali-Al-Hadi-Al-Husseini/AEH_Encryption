...
class Enc:
    ...
    # create a string that is as the size of the text from hashes using sha256
    @classmethod
    def create_list(cls, str key, float size):
        if size <= 64:
            new_key = [key]
            return new_key
    ...
    def split_and_hash(cls, key):
        cdef int half_key = int(len(key) / 2) + 1
    ...
    """ takes the size of the text that to be encrypted  and takes the key to generate 
    the file (keys.txt) using the class method create_file adn then take  the  series of 
    hashes and pass it to the nums classmethod"""
    ...
    """take a key and generates a new list with different order  of the letters 
    (changes the index of each letter and give a new_list)
